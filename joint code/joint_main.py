...
import string
import numpy as np
from joint_data import load_data
from joint_data import load_image_for_test_set
from joint_model import my_model
import h5py
import xlwt, xlrd
...
for k in range(1):
    for i in range(1):
        ...
        # valid_set = five_fold[1]

        data_write("./output/test_dataset" + str(i) + ".xls", test_set)
        ...
        train_data, train_label, train_expression_data, train_expression_label = load_data(train_set, k)
        print('train-set has beed loaded!')

        valid_data, valid_label, valid_expression_data, valid_expression_label = load_data(valid_set, k)
        print('valid_set has beed loaded!')

        test_data, test_label, test_expression_data, test_expression_label = load_image_for_test_set(test_set, k)
        print('test-set has beed loaded!')

        outcome[i, :] = my_model(i, train_data, train_label, valid_data, valid_label, test_data, test_label,
        ...
        del train_data, train_label, train_expression_data, train_expression_label
        del valid_data, valid_label, valid_expression_data, valid_expression_label
        del test_data, test_label, test_expression_data, test_expression_label
